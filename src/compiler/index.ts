...
export default class Complier {
    ...
    private async getDefaultConfig(): Promise<string> {
        return await new Promise((_resolve, reject) => {
            const defaultFiles = glob(this.input + "/_default.arc", (error, matches) => {
                if(matches.length > 0) {
                    const match = matches[0];
                    const arcFileContents = readFileSync(resolve(match)).toString();
                    _resolve(arcFileContents)
                } else {
                    _resolve("");
                }
            });
    ...
    private writeBigFile() {
        ...
        this.getDefaultConfig().then((template) => {
